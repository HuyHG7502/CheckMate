...
namespace CheckMate.Strategies
{
    public class AlphaBetaStrategy : IStrategy
    {
        ...
        public AlphaBetaStrategy()
        {
            ...
            _random = new Random();
            _zobristTable = new ulong[Constants.SQUARE_NUM, 12];    // 6 PieceTypes x 2 PieceColours
        ...
        private Move IterativeAlphaBeta(int alpha, int beta, int depth, PieceColour colour, out int bestScore)
        {
            ...
            foreach (var move in allMoves)
            {
                ...
                int score = -AlphaBeta(-beta, -alpha, depth - 1, Util.GetOpponent(colour));
        ...
        private int AlphaBeta(int alpha, int beta, int depth, PieceColour colour)
        {
            ...
            {
                int r = 2;
                int score = -AlphaBeta(-beta, -beta + 1, depth - 1 - r, Util.GetOpponent(colour));
            ...
            int orgAlpha = alpha;
            PieceColour oppColour = Util.GetOpponent(colour);
        ...
        {
            int standPat = Util.GetScore(_board, colour);
            if (standPat >= beta)
                return beta;
            ...
            foreach (var move in captureMoves)
            {
                ...
                int score = -Quiescence(-beta, -alpha, Util.GetOpponent(colour));
                _board.UndoMove(move);
        ...
        private List<Move> GetOrderedMoves(Board board, PieceColour colour)
        {
            ...
            foreach (var piece in board.Pieces(colour))
                allMoves.AddRange(piece.GetMoves(board, Util.GetSquare(_board, piece)));
        ...
        private List<Move> GetCaptureMoves(Board board, PieceColour colour)
        {
            ...
            foreach (var piece in board.Pieces(colour))
            {
                var moves = piece.GetMoves(board, Util.GetSquare(_board, piece));
