...
namespace CheckMate.Managers
{
    public static class MoveValidator
    {
        ...
        public static bool DetectCheck(Board board, PieceColour colour)
        {
            ...
            PieceColour opp = GameUtil.GetOpponent(colour);
            ...
            {
                if (sqr.IsOccupied() && sqr.Piece.IsMine(opp))
                {
                    List<Move> moves = sqr.Piece.GetMoves(_board, sqr);
                    if (moves.Any(move => move.To == kingSqr))
                        return true;
                }
            }
        ...
        public static bool DetectCheck(Board board, PieceColour colour, Move move)
        {
            ...
            foreach (Square sqr in _board.Squares)
            {
                if (sqr.IsOccupied() && sqr.Piece.IsMine(opp))
                {
                    List<Move> oppMoves = sqr.Piece.GetMoves(_board, sqr);
        ...
        public static bool DetectCheckmate(Board board, PieceColour colour)
        {
            ...
            {
                List<Move> moves = piece.GetMoves(_board, GameUtil.GetSquare(_board, piece));
