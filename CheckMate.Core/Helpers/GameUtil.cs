...
namespace CheckMate.Helpers
{
    ...
    {
        public static Dictionary<PieceType, string> TypeMap { get; } = new()
        {
            { PieceType.King,   "k" },
            { PieceType.Queen,  "q" },
            { PieceType.Rook,   "r" },
        ...
        public static Dictionary<PieceColour, string> ColourMap { get; } = new()
        {
            { PieceColour.White, "w" },
            { PieceColour.Black, "b" },
            { PieceColour.Null,  "" }
        ...
        // Given a player's PieceColour, determine the Opponent's
        public static PieceColour GetOpponent(PieceColour colour)
            => colour == PieceColour.White ? PieceColour.Black : PieceColour.White;

        // Given the mouse position, get the board Square
        public static Square GetSquare(Board board, MouseState mouse)
        {
            int x = (mouse.X - Layout.BoardOffsetX) / Layout.SquareSize;
            int y = (mouse.Y - Layout.BoardOffsetY) / Layout.SquareSize;

            Square sqr = new(x, y);
            return sqr.IsInBounds(0, Layout.SquareCount) ? board[sqr] : Square.Null;
        }

        // Given a piece, get its board Square
        public static Square GetSquare(Board board, Piece piece)
        {
            foreach (var sqr in board.Squares)
                if (sqr.IsOccupied() && sqr.Piece == piece)
                    return sqr;
        ...
        public static string ToPieceNotation(PieceColour colour, PieceType type)
            => ColourMap[colour] + TypeMap[type];

        public static string ToPieceNotation(this Piece piece)
            => ToPieceNotation(piece.Colour, piece.Type);
    }
}
