        ...
        public override List<Move> GetMoves(Board board, Square fromSqr)
        {
            ...
            if (!fromSqr.IsNull() && fromSqr.IsOccupied())
            {
                ...
                if (toSqr.IsInBounds(0, Layout.SquareCount))
                {
                    ...
                    {
                        if (new Square(toSqr + captureDir).IsInBounds(0, Layout.SquareCount)
                    ...
                    foreach (var captureDir in captureDirs)
                    {
                        if (new Square(fromSqr + captureDir).IsInBounds(0, Layout.SquareCount)
                            ...
                            && board[fromSqr + captureDir].Piece is Pawn target)
                        {
                            if (target.IsEnPassant && !target.IsMine(Colour))
                                if (new Square(fromSqr + dir + captureDir).IsInBounds(0, Layout.SquareCount))
